...
function Client(sid, authToken, base_url, opts) {
    if(!(this instanceof Client)) {
        return new Client(sid, authToken, base_url, opts);
    }

    if(!sid || !authToken || !base_url) {
        throw new Error('sid, authToken, and base_url are required');
    }

    // This is a global so that Twiml can access it as well.
    // Probably a better class structure would eliminate this.
    autoUri = new AutoUri(base_url, opts);
